...
const AttendanceReportListview = ({ reports, onOpenDetail, onEdit, onAdd, onDelete }) => {
  const { t } = useTranslation()
  const [fullImageUrl, setFullImageUrl] = useState(null)
  const [emotionModalData, setEmotionModalData] = useState(null)
  const [openDetailModalIndex, setOpenDetailModalIndex] = useState(null)
  ...
  return (
    <>
          ...
          <TableBody>
            {reports.map((r, idx) => (
              <TableRow
                ...
                hover
                sx={{
                  ...(r.isEntryReport && { borderTop: '2px solid #4caf50', borderBottom: '2px solid #4caf50' }),
                  ...(r.isExitReport && { borderTop: '2px solid #f44336', borderBottom: '2px solid #f44336' })
                }}
              >
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, alignItems: 'center' }}>
                    ...
                    {r.isEntryReport && (
                      <Chip
                        label='ورود'
                        size='small'
                        sx={{ bgcolor: '#4caf50', color: '#fff', ml: 1, borderRadius: '9px' }}
                      />
                    )}
                    {r.isExitReport && (
                      <Chip
                        label='خروج'
                        size='small'
                        sx={{ bgcolor: '#f44336', color: '#fff', ml: 1, borderRadius: '9px' }}
                      />
                    )}
                  </Box>
                </TableCell>
                ...
                <TableCell align='center'>
                  <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                    <IconButton
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        if (onOpenDetail) onOpenDetail(idx)
                        setOpenDetailModalIndex(idx)
                      }}
                    >
      ...
      {typeof openDetailModalIndex === 'number' && reports[openDetailModalIndex] && (
        <AttendanceReportDetail
          open={true}
          onClose={() => setOpenDetailModalIndex(null)}
          reportData={reports[openDetailModalIndex]}
          allReports={reports}
          currentIndex={openDetailModalIndex}
          onNavigate={direction => {
            const newIdx = openDetailModalIndex + direction

            if (newIdx >= 0 && newIdx < reports.length) setOpenDetailModalIndex(newIdx)
          }}
...
export default AttendanceReportListview
