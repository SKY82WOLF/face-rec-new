...
import { Card, Box, Typography, Button, Avatar, Divider, IconButton, Chip } from '@mui/material'
...
const AttendanceReportGridCard = ({
  reportData,
  allReports = [],
  index = 0,
  onOpenDetail,
  ...
  // Resolve this card's index within the full array so detail navigation starts from correct position
  const resolvedIndex = React.useMemo(() => {
    if (Array.isArray(allReports) && allReports.length > 0 && reportData && reportData.id) {
      const found = allReports.findIndex(r => r.id === reportData.id)

      return found >= 0 ? found : index
    }

    return index
  }, [allReports, reportData, index])

  // Keep detailIndex in sync when opening the modal
  React.useEffect(() => {
    if (openDetailModal) setDetailIndex(resolvedIndex)
  }, [openDetailModal, resolvedIndex])
  ...
  return (
    <>
      <StyledReportCard
        ...
        sx={{
          ...(reportData.isEntryReport && {
            border: '2px solid #4caf50'
          }),
          ...(reportData.isExitReport && {
      ...
      {/* Detail modal wired from grid card */}

      <AttendanceReportDetail
        open={openDetailModal}
        onClose={() => setOpenDetailModal(false)}
        reportData={allReports && allReports.length > 0 && detailIndex >= 0 ? allReports[detailIndex] : reportData}
        allReports={allReports}
        currentIndex={detailIndex}
        onNavigate={direction => {
          const newIndex = detailIndex + direction

          if (newIndex >= 0 && newIndex < allReports.length) setDetailIndex(newIndex)
        }}
...
export default AttendanceReportGridCard
