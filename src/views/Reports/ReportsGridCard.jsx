...
const ReportsGridCard = ({
  ...
  return (
    <>
      <MotionContainer ref={cardRef} index={index} sx={{ width: '100%' }}>
        <HoverWrapper cardRef={cardRef} imageRef={imageRef} detailsRef={detailsRef}>
          <StyledReportCard mode={currentMode} onClick={() => onOpenDetail && onOpenDetail(index)}>
            <Box
              ref={imageRef}
              data-gsap-image
              sx={{ width: '100%', transform: 'scale(1.04)', opacity: 0.92, willChange: 'transform, opacity' }}
            >
              <ImageCarousel
                images={hoverImages}
            ...
            >
              <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                ...
                <Box
                  sx={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', pt: 1 }}
                >
